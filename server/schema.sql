...
CREATE TYPE charity_status_enum AS ENUM ('pending', 'approved', 'rejected');
CREATE TYPE donation_status_enum AS ENUM ('pending', 'complete', 'failed');
CREATE TYPE payment_method_enum AS ENUM ('mpesa');
...
CREATE TABLE charities (
    ...
    name VARCHAR(255) NOT NULL,
    description TEXT,
    status charity_status_enum DEFAULT 'pending',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
...
CREATE TABLE beneficiaries (
    ...
    name VARCHAR(255) NOT NULL,
    description TEXT,
    inventory_given BOOLEAN DEFAULT FALSE
);
